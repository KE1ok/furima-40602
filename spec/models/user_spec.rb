...
RSpec.describe User, type: :model do
  ...
  describe 'ユーザー新規登録' do
    context '新規登録できる場合' do
      it 'usersテーブルのカラムすべてにデータがあれば登録できる' do
    ...
    context '新規登録できない場合' do
      ...
      it 'passwordに全角文字を含む場合は登録できない' do
        @user.password = 'aｂｃｄeｆ'
        @user.password_confirmation = 'aｂｃｄeｆ'
        @user.valid?
        expect(@user.errors.full_messages).to include('Password must be a mix of letters and numbers')
      end
      ...
      it 'last_rubyがカタカナ以外では登録できない' do
        @user.last_ruby = 'やまだ'
        @user.valid?
        expect(@user.errors.full_messages).to include('Last ruby is invalid')
      end
      it 'first_rubyがカタカナ以外では登録できない' do
        @user.first_ruby = 'りくたろう'
        @user.valid?
